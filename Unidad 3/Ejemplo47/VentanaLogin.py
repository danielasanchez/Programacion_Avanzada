    ...
    def loguear():
        ...
        if usuario.index("end")==0 or contra.index("end")==0:
            ...
            if len(lista)==0:
                ...
                if lista[0].type=="admin":
                    print(f'Bienvenido (a) {lista[0].name}')
                    loginWindow.destroy()
                    #le enviamos al usuario de tipo ADMIN identificado
                    VentanaPrincipal(lista[0])
                else:
                    #el usuario existe pero no tiene acceso a la ventana anterior
                    messagebox.showinfo("Error","Este sistema no es para ti")
                    usuario.focus_set()
                    usuario.delete(0,END)
                    contra.delete(0,END)
